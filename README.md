...
| [`peso_balanza_cam/`](peso_balanza_cam/) | **ESP32-S3-WROOM CAM (N16R8)** | OCR con la cámara **y/o** sniffer + servidor web. Se elige el método en `config.h` (`ENABLE_OCR` / `ENABLE_SNIFFER`); lo desactivado no se compila. |

> Sniffer en **GPIO1 (SL) / GPIO21 (DA)** (la cámara ocupa `GPIO4..18`; GPIO2 tiene LED en placa).
> Arduino IDE: "ESP32S3 Dev Module", **PSRAM "OPI PSRAM"**.
...
## Fase 2 — Firmware final (pendiente)

Con el mapa de la Fase 1:
- Escucha pasiva → reconstruye los 16 bytes en cada refresco.
- Aplica el mapa → PESO, PRECIO, TOTAL, puntos decimales, icono ESTABLE.
- Valida (`TOTAL ≈ PESO × PRECIO`, estable durante N tramas).
- Publica por MQTT / serie con timestamp.
